...
void handle_get_bonds(void)
{
        ...
        if(err != BLE_STATUS_OK)
        {
               uart_print_line("ERROR: %d", err);
        }else
        {
               uart_print_line("EVT_GET_BONDS: %d devices", length);

               uint8_t i = 0;
               for(i = 0; i<length; i++)
               {
                       uart_print_line("EVT_BONDED_DEVICE: slot: %d: addr: %s addr_type: %d bonded: %d connected: %d",
                                       i, format_bd_address(&devices->address), devices->address.addr_type,
                                       devices->bonded, devices->connected);
               }

               uart_print_line("EVT_GET_BONDS_COMPLETE");
        }
